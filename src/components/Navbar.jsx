...
import { Utensils, LogOut, Menu as MenuIcon, X, Calendar, User } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 text-xs font-bold tracking-wider uppercase">
          ...
          {user?.rol === 'administrador' && (
            <Link to="/admin" className="px-3 py-1 rounded-lg bg-[#D16014]/20 border border-[#D16014]/50 text-[#D16014]">
              Dashboard Admin
            </Link>
          )}
        </nav>
        ...
        <div className="hidden md:flex items-center gap-4 text-xs font-bold">
          {user ? (
            <div className="flex items-center gap-3 border-l border-[#F8FFE5]/15 pl-4">
              <span className="text-gray-300 capitalize">{user.email.split('@')[0]}</span>
              <button 
                onClick={logout} 
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#0A090C] border-b border-[#F8FFE5]/10 px-6 py-4 space-y-3 text-xs font-bold uppercase">
          ...
          {user ? (
            ...
            <button onClick={() => navigate('/login')} className="w-full py-2.5 bg-[#D16014] rounded-xl text-center text-white mt-2">Acceso</button>
          )}
        </div>
      )}
    </header>
